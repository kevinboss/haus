...
public sealed class HelperCreateSelectCommand(IAuthService auth, IHassWebSocketClient ws)
    : HausCommand<HelperCreateSelectCommand.Settings>(auth)
{
    ...
    protected override Task<int> RunAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        return HelperCreator.CreateAsync(ws, HelperKind.Select, settings.Name, settings.ObjectId, settings.Icon, body, settings, cancellationToken);
    }
}
